...
def courseItemHandler(request, course_id):
    ...
    return render(request, 'course_item.html', {'user_id': user_id,
                                                'active_user': active_user,
                                                'course': course,
...
def apiCoursesEditHandler(request, item_id):
    ...
    return render(request, 'api/courses_item_edit.html', {'item': item, 'categories':categories})


def apiNewsHandler(request):
    if request.POST:
        action = request.POST.get('action', '')
        id = int(request.POST.get('id', 0))
        if action == 'delete':
            nw = News.objects.get(id=id)
            nw.delete()
    news = News.objects.all()
    return render(request, 'api/news.html', {'news':news})


def apiNewsAddHandler(request):
    if request.POST:
        action = request.POST.get('action', '')
        title = request.POST.get('title', '')
        ...
        if action == 'add':
            nw = News()
            nw.title = title
            nw.mini_desc = mini_desc
            nw.desc= desc
            nw.views_count = views_count
            nw.logo = logo
            nw.create_datetime = date
            nw.save()

    return render(request, 'api/news_item.html', {})


def apiNewsEditHandler(request, item_id):
    return render(request, 'news_item_edit.html', {})
